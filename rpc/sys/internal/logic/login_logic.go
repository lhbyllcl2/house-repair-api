...
var (
	ErrorUserIsNo      = errors.New("用户不存在")
	ErrorWrongPassword = errors.New("用户密码不正确")
)
...
func (l *LoginLogic) Login(in *sys.LoginReq) (*sys.LoginResp, error) {
	...
	case sqlc.ErrNotFound:
		...
		return nil, ErrorUserIsNo
	default:
		logx.WithContext(l.ctx).Errorf("用户登录失败,参数:%s,异常:%s", in.UserName, err.Error())
		return nil, err
	}
	if !password.ComparePassword(in.Password, userInfo.Salt, userInfo.Password) {
